...
/* app.post('/api') is used to to get the asset for the calendar of the user. */
...
/* readJSONFileForLogin is used to read the {login}ADE.json to help for the app.post('/api')*/
    ...
    app.get('/test2', async (req, res) => {
        try {
            ...
            if (is2AMParisTime) {
                // Process plannings data only at 2 AM
                ...
                for (const file of files) {
                    ...
                    if (buildingName) {
                        ...
                        every_slot.push({ [formattedBuildingName]: slot });
                    }
                }
                
                // Write aggregated data to every_slot.json
                await fsPromises.writeFile('./src/out/every_slot.json', JSON.stringify(every_slot, null, 2));
            }
            // Regardless of the time, send the existing or updated data as the response
            const existingData = await fsPromises.readFile('./src/out/every_slot.json', 'utf8');
            res.json(JSON.parse(existingData));
        } catch (error) {
            console.error("Error processing test2:", error);
            res.status(500).json({ error: "An error occurred while processing test2." });
        }
    });
